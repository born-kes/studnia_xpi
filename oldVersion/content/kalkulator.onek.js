// ==UserScript==
// @name				Kalkulator czasĂłw ataku
// @author				Szatdafakap
// @namespace			http://aturime.pl
// @description			Wersja dla Plemiona 7.1
// @include			http://pl*.plemiona.pl/game.php*type=own_home*
...
function createField() {
	...
	if (tab.innerHTML.indexOf("Lucznik") >= 0) {
	...
	field_ambit.value = "Kiedy wyslac";
	cell.appendChild(field_ambit);
	//field_ambit.setAttribute("onclick", "calculateSendTimes();");

	cell.innerHTML += " Czy kolorowac? ";
	var field_ambit = _$("input");
	field_ambit.type = "checkbox";
	field_ambit.id = "unitColor";
//	field_ambit.value = "Czy kolorowac?";
	cell.appendChild(field_ambit);
...
function calculateSendTimes() {
	...
	var unitColor = gid("unitColor").checked;
		...
		if (tbodies[i].style.display != "none") {
			...
			if (row.lastChild.previousSibling.textContent == "Rozkazy"){
				...
				row.appendChild(cell);
					...
					if(unitColor){
					row.lastChild.innerHTML = 	 G_green_red(launchTime.format("H"),launchTime.format("d.m\u00a0H:i:s:x"));						
						}else{
...
function fillSaved() {
	...
	gid("unitColor").checked = GM_getValue("unitColor") != false ? GM_getValue("unitColor") : false;
}
...
function saveForm() {
	...
	GM_setValue( 'unitColor', gid("unitColor").checked);
}
			...
			for( j = i; j < i + this.batchSize && j < this.tbodies.length; j+=1 ) {
				...
				if ( arrivalTime.getTime() >= beginTime.getTime() && arrivalTime.getTime() <= endTime.getTime() && checkArmy(tbodies[j]) ) {
					...
					if (row.lastChild.previousSibling.textContent == "Rozkazy"){
						...
						if(unitColor){
					row.lastChild.innerHTML = 	 G_green_red(arrivalTime.format("H"),arrivalTime.format("d.m\u00a0H:i"))						
						}else{
...
function opuznieniexy(){	
	...
	newCell.innerHTML = "<a href='/game.php?screen=overview_villages&mode=units&type=own_home"+tid + "&coords=" + coords + "'>» Wpisz do kalkulatora</a>"
