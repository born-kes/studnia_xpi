// ==UserScript==
// @name           Filtrowanie rozkazÛw
// @namespace      http://code.google.com/p/plemiona-skrypty/
// @description    Filtrowanie rozkazÛw - bez wykrywania fake'Ûw. Wymaga konta Premium.
// @version        1.9.0001
...
function addLinks() {
        ...
        for (i = tables.length-1; i >= 0; i--) {
                if ( tables[i].getElementsByTagName('tr').length > 1 && trim(tables[i].getElementsByTagName('tr')[1].textContent).substring(0, 11) == "WspÛ≥rzÍdne") {
...
function createVillagesList() {    //  alert(document.URL.indexOf("mode=inco"));
        ...
        for (i = 1; i < rows.length-1; i++) {
                ...
                if (order.match(/attack.png/)) {// "Atak na"
                        ...
                        village = orders;
                } else if (order.match(/return.png/)  ) { //"PowrÛt "
                        village = orders;
                } else if (order.match(/support.png/) ) { //"Wsparci"
                        village = orders;
                } else if (order.match(/back.png/)    ) { //"OdwrÛt "
                        village = orders;
